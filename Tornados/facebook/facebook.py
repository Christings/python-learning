...
class Application(Tornados.web.Application):
    def __init__(self):
        ...
        Tornados.web.Application.__init__(self, handlers, **settings)
...
class BaseHandler(Tornados.web.RequestHandler):
    def get_current_user(self):
        ...
        if not user_json:
            return None
        return Tornados.escape.json_decode(user_json)


class MainHandler(BaseHandler, Tornados.auth.FacebookGraphMixin):
...
class AuthLoginHandler(BaseHandler, Tornados.auth.FacebookGraphMixin):
    async def get(self):
        my_url = (
            ...
            + self.request.host
            + "/auth/login?next="
            + Tornados.escape.url_escape(self.get_argument("next", "/"))
        )
        if self.get_argument("code", False):
            ...
            self.set_secure_cookie("fbdemo_user", Tornados.escape.json_encode(user))
...
class AuthLogoutHandler(BaseHandler, Tornados.auth.FacebookGraphMixin):
    def get(self):
        self.clear_cookie("fbdemo_user")
        self.redirect(self.get_argument("next", "/"))


class PostModule(Tornados.web.UIModule):
    def render(self, post):
        return self.render_string("modules/post.html", post=post)


def main():
    Tornados.options.parse_command_line()
    if not (options.facebook_api_key and options.facebook_secret):
        print("--facebook_api_key and --facebook_secret must be set")
        return
    http_server = Tornados.httpserver.HTTPServer(Application())
