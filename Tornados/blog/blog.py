...
class EntryHandler(BaseHandler):
    async def get(self, slug):
        ...
        if not entry:
            raise Tornados.web.HTTPError(404)
...
class AuthCreateHandler(BaseHandler):
    ...
    async def post(self):
        if await self.any_author_exists():
            raise Tornados.web.HTTPError(400, "author already created")
        hashed_password = await Tornados.ioloop.IOLoop.current().run_in_executor(
            None,
            bcrypt.hashpw,
            Tornados.escape.utf8(self.get_argument("password")),
            bcrypt.gensalt(),
        )
        author = await self.queryone(
            ...
            Tornados.escape.to_unicode(hashed_password),
        )
...
class AuthLoginHandler(BaseHandler):
    ...
    async def post(self):
        ...
        hashed_password = await Tornados.ioloop.IOLoop.current().run_in_executor(
            ...
            bcrypt.hashpw,
            Tornados.escape.utf8(self.get_argument("password")),
            Tornados.escape.utf8(author.hashed_password),
        )
        hashed_password = Tornados.escape.to_unicode(hashed_password)
...
class EntryModule(Tornados.web.UIModule):
...
async def main():
    ...
    async with aiopg.create_pool(
        ...
        shutdown_event = Tornados.locks.Event()
        await shutdown_event.wait()


if __name__ == "__main__":
    Tornados.ioloop.IOLoop.current().run_sync(main)
