...
import Tornados.web
from Tornados import options


class POSTHandler(Tornados.web.RequestHandler):
    def post(self):
...
@Tornados.web.stream_request_body
class PUTHandler(Tornados.web.RequestHandler):
    def initialize(self):
        self.bytes_read = 0
...
def make_app():
    return Tornados.web.Application([(r"/post", POSTHandler), (r"/(.*)", PUTHandler)])
